Посилання на гітхаб: https://github.com/gbfgb-a11y/-_nosql_2.git
...
1.2:
    ...
    2: 
        Спочатку за завданням я мав використовувати allenai/specter2_base – вона спеціально навчена на наукових статтях, розуміє терміни, цитування. На HuggingFace пишуть, що вона призначена для семантичного пошуку, визначення близькості документів і передбачення цитувань. Але біда в тому, що на Windows вона потребує адаптерів і створення симлінків, а в моїй системі це не працювало навіть після всіх танців з бубном.
        Тому я вирішив не мучитися і взяв універсальну модель all-MiniLM-L6-v2. Вона не така точна для наукового тексту, але працює без проблем, дає вектори розмірністю 384, і для навчального завдання її цілком достатньо. Крім того, вона швидша, що важливо, коли ти 10 разів перезапускаєш скрипти, в мене одна генерація ембедінгу займае 30-40 хв.
    3:
        На картці моделі all-MiniLM-L6-v2 рекомендують косинусну схожість. Це стандарт для семантичного пошуку. Тому в Pinecone я створив індекс з параметром metric="cosine". Також у коді я додав normalize_embeddings=True, щоб вектори були одиничної довжини тоді косинусна схожість обчислюється як скалярний добуток, що набагато швидше.
        Чому це важливо? Якби я випадково обрав metric="euclidean", то база даних сортувала б вектори інакше, і результати пошуку не відповідали б тому, як модель «бачить» схожість текстів. Тому потрібно, щоб метрика в індексі збігалася з тією, на яку модель навчена.
